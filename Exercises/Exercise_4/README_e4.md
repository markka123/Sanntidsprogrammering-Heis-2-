...
ANSWER: "Aiming to detect success in results ensures that the system is woriking properly. That is, because there are multiple ways the system could not do as expected while not directly failing. It also allows the system to not explicitly define all possible errors/failure modes, but instead be robust against all errors even unknown."
...
ANSWER: "First and formost this approach ensures that the program will return to a known good state in the event of any failures. There are many types of errors that the system may not be able to fix entirely and which therefore renders it in an incomplete state. It is also possible to envision a scenario where the programs attempt to restore itself introduces new failures."
...
"A separate supervisor introduces a single point of failure, which is not the case for process pairs. In addition, process pairs are already running and can therefore takeover more efficiently than in the case where the supervisor need to spin up a new instance."
