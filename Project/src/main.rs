...
fn main() -> std::io::Result<()> {
    ...
    let (port, elev_id) = fetch_command_line_args();

    println!("{}, {}", port, elev_id);
...
pub fn fetch_command_line_args() -> (u16, u8) {
    ...
    let elevator_id = if command_line_args.len() > 2 {
        match command_line_args[2].parse::<u8>() {
            Ok(id) => id,
            Err(_) => {
                eprintln!("Warning: Invalid elevator ID provided. Using default: {}", default_elev_id);
                default_elev_id
            }
        }
    } else {
