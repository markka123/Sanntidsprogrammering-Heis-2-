...
pub fn distributor(
    ...
    let mut unconfirmed_orders: Vec<(u8, poll::CallButton)> = Vec::new();
        ...
        cbc::select! {
            ...
            recv(order_completed_rx) -> order_completed => {
                ...
                if states[elevator_id as usize].offline {
                    ...
                    all_orders.remove_order(order_completed.clone(), elevator_id);
                    unconfirmed_orders.retain(|(msg, order)| *msg != msg_type || order.floor != order_completed.floor || order.call != order_completed.call);
                    new_order_tx.send((offline_orders, all_orders.hall_orders)).unwrap(); 
                }
                call_msg_tx.send((msg_type, order_completed)).unwrap();
            },
            recv(unconfirmed_orders_ticker) -> _ => {
                unconfirmed_orders.iter().for_each(|(msg_type, call)| {
            ...
            recv(message_rx) -> message => {
                match message {
                    Ok(Message::CallMsg(call_msg)) => {
                        ...
                        match msg_type {
                            NEW_ORDER => {
                                all_orders.add_order(new_order, id);
                            },
                            COMPLETED_ORDER => {
                                all_orders.remove_order(new_order, id);
                            },
                            _ => {
                                //Handle error
                            }
                        ...
                        if !elevator_is_availible {
                            if let Some(new_assigned_orders) = all_assigned_orders_map.get(&elevator_id) {
                                if (*new_assigned_orders != assigned_orders)  || (hall_orders != all_orders.hall_orders) {
                                    ...
                                    all_orders.hall_orders = hall_orders;
                                    new_order_tx.send((assigned_orders, all_orders.hall_orders)).unwrap();
                                }
                            } 
                        }
                        confirm_orders(&mut unconfirmed_orders, &all_assigned_orders_map, elevator_id);                        
                    }
                    Err(e) => {
                    }
                }
            ...
            recv(is_online_rx) -> is_online_msg => {
                ...
                if network_status && !is_online {
                    ...
                    for (order_type, order) in unconfirmed_orders.iter() {
                    ...
                    is_online = false;
                    all_orders.hall_orders = [[false; 2]; config::ELEV_NUM_FLOORS as usize];
                    new_order_tx.send((offline_orders, all_orders.hall_orders)).unwrap();
                }
            }
        }
...
fn confirm_orders(
    unconfirmed_orders: &mut Vec<(u8, poll::CallButton)>,
    all_assigned_orders_map: &HashMap<u8, [[bool; 3]; config::ELEV_NUM_FLOORS as usize]>, 
    elevator_id: u8,
) {
    unconfirmed_orders.retain(|(order_type, order)| {
